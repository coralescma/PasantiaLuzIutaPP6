...
// CAPTURAR MENSAJE DE ÉXITO TRAS REDIRECCIÓN (Evita duplicados por F5)
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['btn_guardar_venta'])) {
    if (!$id_jornada) {
        ...
        if ($conn->query($sql_trans)) {
            ...
            // --- SOLUCIÓN AL ERROR GRAVÍSIMO: REDIRECCIÓN POST-VENTA ---
...
<head>
    ...
    <style>
        ...
        option.oculto { display: none; }
        .btn-delete { background:none; border:none; color:#e11d48; cursor:pointer; font-size: 1.2rem; }
    </style>
</head>
<body>
    ...
    <script>
        ...
        function agregarFila() {
            ...
            tr.innerHTML = `
                <td><select class="p-select" required onchange="actualizarSeleccion(this)">${options}</select></td>
                ...
                <td><button type="button" class="btn-delete" onclick="eliminarFila(this)">✕</button></td>
            `;
            tbody.appendChild(tr);
            filtrarOpciones();
        }

        function actualizarSeleccion(select) {
        ...
        function filtrarOpciones() {
            const seleccionados = Array.from(document.querySelectorAll('.p-select'))
                                      .map(s => s.value)
                                      .filter(v => v !== "");
            ...
            document.querySelectorAll('.p-select').forEach(select => {
                ...
                Array.from(select.options).forEach(option => {
                    ...
                    if (seleccionados.includes(option.value) && option.value !== valorActual) {
                        option.classList.add('oculto');
                    } else {
